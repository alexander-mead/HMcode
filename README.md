...
```HMcode``` should compile with any ```Fortran``` compiler, and you can change the compiler within the ```Makefile```. ```HMcode``` requires my library (https://github.com/alexander-mead/library) to be downloaded in order to work and you will need to change the ```Makefile``` to point to the ```src``` directory within the directory (e.g., ```MOD_DIR = /path/to/library/src``` in the ```Makefile```) where you have downloaded the library. Note well that you do **not** need to compile the library itself in order for ```HMcode``` to run -- ```HMcode``` just needs to know the location of the source files within the library. Once the required edits to the ```Makefile``` have been implemented, simply type ```>make```. To run the compiled code type ```>./bin/HMcode```.
